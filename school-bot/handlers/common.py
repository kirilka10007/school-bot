...
def build_application_text(data: dict) -> str:
    ...
    return (
        ...
        f"📚 <b>Тип занятия:</b> {data.get('lesson_type', '-')}\n"
...
def build_cabinet_text(student_name: str, directions: list[tuple]) -> str:
    ...
    for index, direction in enumerate(directions, start=1):
        ...
        lines.append(
            ...
            f"   Остаток: <b>{lesson_balance}</b> | Тариф: {tariff_type}"
        )

    lines.extend(["", "Если данные неактуальны, напишите администратору."])
    return "\n".join(lines)
...
def build_multi_students_warning(students_count: int) -> str:
    ...
    return (
        ...
        "Сейчас показана самая свежая запись."
    )
...
def build_payment_caption(
    ...
    text = (
        ...
        f"🔗 <b>Username:</b> {username if username else 'нет username'}\n"
...
async def show_main_menu(message_obj: Message, state: FSMContext):
    ...
    await message_obj.answer(
        "Выбери нужный раздел:", reply_markup=get_main_menu_keyboard()
    )
...
async def send_teacher_card(
    ...
    await state.update_data(
        selected_teacher_subject=subject, selected_teacher_index=index
    )
